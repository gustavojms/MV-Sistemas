...
public interface ItemAssignmentRepository extends JpaRepository<ItemAssignment, Long>{
    ...
    boolean existsItemAssignmentByCoffeeDayIdAndItemId(Long itemId, Long coffeeDayId);
    ...
    @Query(value = "SELECT * FROM TB_ITEM_ASSIGNMENT WHERE ID_USER = :userId AND COFFEE_DAY_ID = :coffeeDayId", nativeQuery = true)
    List<ItemAssignment> findItemAssignmentByUserIdAndCoffeeDayId(Long userId, Long coffeeDayId);
